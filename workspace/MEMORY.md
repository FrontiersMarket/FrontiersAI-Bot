...
Load this file **only in main session** (direct chat with your human). Do NOT load in shared contexts (Discord, group chats) — security risk.
...
Always add `WHERE ranch_uuid = '<scoped_uuid>'` to every query on these tables. No exceptions — even if the user asks for "all" records.
